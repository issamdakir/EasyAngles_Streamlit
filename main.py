...
def main():
    ...
    if selected_box == 'CAMERA CALIBRATION TOOL':
        ...
        with st.form("my-form", clear_on_submit=True):
            ...
            CalibFiles = st.file_uploader("", type=['png', 'jpg','tif','tiff','TIF','TIFF', 'jpeg','bmp','webp','pfm','sr','ras','exr','hdr','pic'],key=st.session_state.key, help="EasyAngles Memory over loaded !", accept_multiple_files=True)
            submitted = st.form_submit_button('CALIBRATE CAMERA')
            

        # if submitted and CalibFiles :
        if submitted and CalibFiles :

            Processing = st.empty()
            Processing.text('Please wait while processing, results will be displayed within few secondes...')
            res, message,CHARUCO_BOARD,image_size, corners_all, ids_all = Calibrate(CaseLength,MarkerLength,CalibFiles,Processing)
            if res :
        ...
        # if CalibFiles :
        #     # print('calibFiles before pop :', CalibFiles) 
        #     # f =  CalibFiles.pop()
        #     # print('pop file :', f) 
        #     # print('calibfiles after pop :', CalibFiles) 
        #     st.session_state.pop('key')
        #     st.experimental_rerun()
        #     try :
        #         for fData in CalibFiles:
        #             print(fData) 
        #     except Exception :
        #         print('my Error')
        #     # print('after upload :')
